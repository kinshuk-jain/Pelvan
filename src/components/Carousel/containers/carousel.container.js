...
export const LightCarouselContainer = ({
  children,
  containerStyle,
  ...props
}) => (
...
export const ButtonsContainer = ({
  ...
  <div {...props} style={{ position: 'relative', ...buttonsContainerStyle }}>
    {children}
  </div>
);
...
class LightCarousel extends Component {
  ...
  */
  initCarousel = () => {
    ...
    this.autoWidth =
      (this.slideshowClientWidth + gap) / this.slidesToShow - gap;
    this.displacement = (slideWidth || this.autoWidth) + gap;
    this.currentPosition = infinite
      ? -(this.displacement * this.slidesToShow)
      : 0;
    ...
    if (infinite && stopOnHover && autoplay)
      this.setMouseEvents(this.clearAutoplay, this.setAutoplay);

    /*  Set the right amount of sliders to shown, given the current device width */
    this.setResizeEvent();
    /* Move the position to the first original slide */
    this.slidesContainer.style.transform = `translateX(${this
      .currentPosition}px)`;
    if (setSlide > 0) {
  ...
  */
  setEndTransition = eventFunction => {
    ...
    const endTransition = Object.entries(endTransitions).find(
  ...
  */
   ...
   */
  calculateTotalSlides = (slidesToShow, elements) =>
    slidesToShow * 2 + elements;
  ...
  */
  addClones = (array, items) => {
    ...
    for (let i = 0; i < items; i += 1) {
      array.unshift(array[array.length - i - (items + 1)]);
    }
    return array;
  };
  ...
  */
  isLastSlide = () => {
    const isIt =
      this.state.currentSlide >= this.totalUniqueElements - this.slidesToShow;
    return isIt;
  };
  ...
  move = (to = 'right') => {
    ...
    if (!isNaN(to) && to > 0) {
      const fallbackPosition =
        this.slideshowWidth - this.displacement * this.slidesToShow;
      newDisplacement *= to;
      const finalPosition =
        newDisplacement > fallbackPosition ? fallbackPosition : newDisplacement;
  ...
  */
  resetPositionTo = direction => {
    ...
    if (direction === 'start') {
      ...
      newPosition = -(
        this.displacement *
        (this.totalUniqueElements + this.slidesToShow - 1)
      );
    }
  ...
  */
  hasAllItemsVisible = () =>
    this.state.currentSlide === 0 && this.isLastSlide();
  ...
  */
  renderSlides = children => {
     ...
     * if there is only one child, it won't be an array of one.
     */
    const slides = React.Children
      .toArray(children)
      .map((slide, i) => ({ slide, id: i }));
    const clientWidth = R.path(['clientWidth'], this.slideshow) || 0;
    const slideshowClientWidth =
      clientWidth >= 0 ? clientWidth : this.slideshowClientWidth;
  ...
  render() {
    ...
    return (
      ...
      >
        {!this.hasAllItemsVisible() && (
          <Button onClick={() => this.moveBackward()}>{prevBtn}</Button>
        )}
        <Slideshow
        ...
        {!this.hasAllItemsVisible() && (
          <Button onClick={() => this.moveForward()}>{nextBtn}</Button>
        )}
      </LightCarouselContainer>
    );
  }
